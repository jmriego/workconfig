function mouseClick(button, point, modifiers)
    ...
    local current_point = hs.mouse.getAbsolutePosition()
    ...
    hs.timer.doAfter(0.1,
        function()
            hs.eventtap.event.newMouseEvent(hs.eventtap.event.types[(button .. "MouseUp")], point):setProperty(clickState, 1):post();
            hs.eventtap.event.newMouseEvent(hs.eventtap.event.types["mouseMoved"], current_point):post();
        end)
end
